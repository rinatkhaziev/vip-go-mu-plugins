...
class WP_Filesystem_VIP_Test extends WP_UnitTestCase {
	...
	public function setUp(): void {
		...
		$this->filesystem->set_local_files( [ 'file1.txt', 'file2.txt' ] );
	...
	public function test_set_local_files() {
		$local_files = [ 'file1.txt', 'file2.txt' ];
		$this->filesystem->set_local_files( $local_files );

		$reflection = new \ReflectionClass( $this->filesystem );
		$property = $reflection->getProperty( 'local_files' );
		$property->setAccessible( true );

		$this->assertEquals( $local_files, $property->getValue( $this->filesystem ) );
	}
}
